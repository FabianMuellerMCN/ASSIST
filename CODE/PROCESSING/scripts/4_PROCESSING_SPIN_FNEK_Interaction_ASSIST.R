...
odir =  paste0(PROCESSING_PATH, 'output/', gsub("-", "_", Sys.Date()), "_", "FNEK_interaction/")# output-directory
dir.create(odir)
...
alldirs = c(odir, sdir, idir_LABKEY, idir_MAIN, idir_RATINGS, idir_QUESTIONNAIRES_CORT)
setwd(odir)
...
names_DV <- names_DV[!names_DV %in% c("FNEK_Score", "SPIN_Score")]

for(v in unique(datAnalyse$VP_Nr)){
  #v = 103
  datAnalyse[datAnalyse$VP_Nr == v & datAnalyse$Intervention_Phase %in% c("post1", "post2"), "SPIN_Score"] <-  datAnalyse[datAnalyse$VP_Nr == v & datAnalyse$Intervention_Phase == "pre", "SPIN_Score"]
  datAnalyse[datAnalyse$VP_Nr == v & datAnalyse$Intervention_Phase %in% c("post1", "post2"), "FNEK_Score"] <-  datAnalyse[datAnalyse$VP_Nr == v & datAnalyse$Intervention_Phase == "pre", "FNEK_Score"]
}
...
for (j in names_DV[!names_DV %in% "socImm_Score"]){   # do not run the models for social immersion Score because this is only available at one time point and only for treatment group
  ...
  datAnalyse_j <- datAnalyse[, c("VP_Nr", "SPIN_Score", "FNEK_Score", "Age", "Sex", "Condition", "Intervention_Phase", name_DV)]
  ...
  if (!name_DV %in% c("IPQ_Score", "USX_Score", "selfExp_totalDuration")){    # measures only assessed once --> directly proceed to post-hoc-testing
    ...
    for (i in c("Value", "Std.Error", "t-value", "r")){ 
      ...
      #res[res$IVs == "factor(Condition):factor(Intervention_Phase)",i] <- res2[res2$IVs == "factor(Condition)1:factor(Intervention_Phase)pre", i]
    }
  ...
  if (posthoc == F) {
    ...
    for (y in c(unique(as.character(datAnalyse_j$Intervention_Phase)))){
      ...
      if (y %in% c("post1", "post2") & "pre" %in% unique(datAnalyse_j$Intervention_Phase)){
        ...
        # side analyses
        lme_mod_sub <- lme(DV_sub ~  Age + factor(Sex) + FNEK_Score + factor(Condition) + IV_baseline, random=~1|VP_Nr, data=dat_sub ,control=lmeControl(opt="optim"), na.action="na.omit")
        ...
        DV_sub = dat_sub [,name_DV]
        # side analyses
        lme_mod_sub <- lme(DV_sub ~  Age + factor(Sex) + FNEK_Score + factor(Condition), random=~1|VP_Nr, data=dat_sub ,control=lmeControl(opt="optim"), na.action="na.omit")
...
datPlot <- datAnalyse
whichVars_plot <- names_EYETRACKING
p <- lapply(c(whichVars_plot),plot_BF)
p <- append(p,list(legend))
